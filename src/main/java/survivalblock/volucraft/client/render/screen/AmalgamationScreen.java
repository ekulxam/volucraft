...
public class AmalgamationScreen extends AbstractContainerScreen<AmalgamationMenu> {
    ...
    public int getHovered3DSlot(double mouseX, double mouseY) {
        ...
        // 1. Boundary check for the 150x150 region
        if (mouseX < xo || mouseX > xo + 150 || mouseY < yo || mouseY > yo + 150) {
            return -1;
        }

        // 2. Direct mapping matching your 2D screen metrics
        // Top-Left is (-1, -1), Bottom-Right is (1, 1)
        float ndcX = (float) (((mouseX - xo) / 150.0) * 2.0 - 1.0);
        float ndcY = (float) (((mouseY - yo) / 150.0) * 2.0 - 1.0);

        // 3. Orthographic scale conversion matching 11F viewports
        float orthoScale = 11.0F / 8.0F;

        // We shoot the ray forward along the Z axis (from Z=-10 to Z=10)
        Vector3f rayOrigin = new Vector3f(ndcX * orthoScale, ndcY * orthoScale, -10.0F);
        Vector3f rayDir = new Vector3f(0, 0, 1);

        // 4. Trace backward through your Renderer's Matrix transformations
        float renderCenter = centerFromScale(11F); // 6.5F
        float pivotY = 9.0F / 16.0F;               // 0.5625F

        // Undo the center translation offset
        ...
        // 6. Account for the LivingEntity mulPose(flip) matrix
        // Because flip multiplies Y by -1 and Z by -1, we reverse the ray orientations
        rayOrigin.y = -rayOrigin.y;
        rayOrigin.z = -rayOrigin.z;
        rayDir.y = -rayDir.y;
        rayDir.z = -rayDir.z;

        // 7. Raycast collision loop testing against the slots
        ...
        for (int i = 0; i < Volucraft.SLOTS; i++) {
            ...
            // Bounding limits generated by transformByIndex
